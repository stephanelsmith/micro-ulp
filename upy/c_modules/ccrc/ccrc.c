...
// crc16
// INPUT
//      buff_in_obj: bytearray, bytes, memoryview
// OUTPUT
//      returns U16 crc
static mp_obj_t crc16(mp_obj_t buff_in_obj) {
	mp_buffer_info_t bufinfo;
	mp_get_buffer(buff_in_obj, &bufinfo, MP_BUFFER_READ);
    uint8_t *buf = bufinfo.buf;
    uint16_t i;

    uint16_t crc = 0;
    for(i=0; i<bufinfo.len; i++){
        crc = ((crc<<8)&0xff00) ^ CRC16_XMODEM_TABLE[((crc>>8)&0x00ff)^buf[i]];
    }
    return mp_obj_new_int(crc);
}
// Define a Python reference to the function above.
static MP_DEFINE_CONST_FUN_OBJ_1(crc16_obj, crc16);

static const uint16_t CRC16_AX25[] = {
...
// OUTPUT
//      returns U16 crc
static mp_obj_t crc16_ccit(mp_obj_t buff_in_obj) {
	mp_buffer_info_t bufinfo;
	mp_get_buffer(buff_in_obj, &bufinfo, MP_BUFFER_READ);
    uint8_t *buf = bufinfo.buf;
    uint16_t i;
    ...
    uint16_t crc = 0xffff;
    for(i=0; i<bufinfo.len; i++){
        crc = ((crc) >> 8) ^ CRC16_AX25[((crc) ^ buf[i]) & 0xff];
    }
    crc = (crc ^ 0xffff) & 0xffff;
    return mp_obj_new_int(crc);
}
// Define a Python reference to the function above.
static MP_DEFINE_CONST_FUN_OBJ_1(crc16_ccit_obj, crc16_ccit);
...
MP_REGISTER_MODULE(MP_QSTR_ccrc, cmodule_ccrc);
